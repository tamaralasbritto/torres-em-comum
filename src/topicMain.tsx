...
function App(){
 ...
 return <main>
  ...
  <section id="consulta" className="consult topicconsult"><div className="sectionhead"><div><p className="eyebrow">CONSULTA POR ASSUNTOS</p><h2>{answered} de {decisionTopics.length} decisões</h2></div><span>~{minutes} min restantes</span></div><div className="progress"><i style={{width:`${progress}%`}}/></div><div className="topiclayout"><aside className="topicnav">{[...new Set(decisionTopics.map(t=>t.category))].map(cat=><div key={cat}><strong>{cat}</strong>{decisionTopics.map((t,i)=>t.category===cat&&<button key={t.id} className={`${i===index?'active':''} ${topicAnswered(t)?'done':''}`} onClick={()=>go(i)}>{topicAnswered(t)?'✓ ':''}{t.title}</button>)}</div>)}</aside><article className={`device topiccard ${!canEdit?'locked':''}`}><div className="crumb">{current.category}</div><h3>{current.title}</h3><p className="topicsummary">{current.summary}</p><p className="articlecount">Relaciona {currentDevices.length} dispositivo(s) da minuta · arts. {[...new Set(currentDevices.map(d=>d.article))].join(', ')}</p><details className="originaldetails"><summary>Ver dispositivos da minuta ({currentDevices.length})</summary><div className="originalstack">{currentDevices.map(d=><article key={d.id}><strong>Art. {d.article}{d.subdivision?`, ${d.subdivision}`:''}</strong>{d.plainLanguage&&<p className="plain">{d.plainLanguage}</p>}<p className="legal">{d.text}</p>{d.translationNote&&<aside><strong>Atenção à redação:</strong> {d.translationNote}</aside>}</article>)}</div></details>{!canEdit&&!finalReceipt&&<div className="gate"><strong>Salve sua identificação acima para responder.</strong></div>}<div className="options topicoptions">{choices.map(c=><button disabled={!canEdit} key={c.id} className={currentResponse?.choice===c.id?'selected':''} onClick={()=>setTopicResponse(c.id)}><strong>{c.label}</strong><small>{c.hint}</small></button>)}</div>{currentResponse&&['change','exclude','observe'].includes(currentResponse.choice)&&<label className="comment">Explique seu ponto<textarea disabled={!canEdit} value={currentResponse.comment||''} onChange={e=>setComment(e.target.value)} placeholder="Escreva em linguagem simples. Não precisa redigir juridicamente."/><small>Evite dados pessoais seus ou de terceiros.</small></label>}<div className="topicpager"><button disabled={index===0} onClick={()=>go(index-1)}>← Anterior</button><button className="skip" disabled={index===decisionTopics.length-1} onClick={skip}>{currentResponse?'Próximo →':'Pular por enquanto →'}</button></div></article></div></section>
  ...
  <footer className="no-print"><p><strong>Torres em Comum</strong><br/>Construção participativa do Regimento Interno</p><p>Os assuntos são uma camada de navegação. O registro auditável continua vinculado aos dispositivos originais da minuta.</p></footer>
 </main>
}
