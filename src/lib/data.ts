...
export type RemoteSession = { participantId:string; protocolId:string; resumeToken:string; status:'draft'|'finalized'; lastSavedAt?:string };
export type FinalReceipt = { protocolId:string; status:'finalized'; finalizedAt:string; payloadHash:string };
...
export type PublicPanel = { k:number; participatingUnits:number|null; devices:PublicPanelDevice[] };
...
function serializeResponses(value:Responses){
...
export async function createRemoteDraft(participant:Participant):Promise<RemoteSession>{
 const result=await api<any>({action:'create',...participant});
 const session:RemoteSession={participantId:result.participantId,protocolId:result.protocolId,resumeToken:result.resumeToken,status:'draft',lastSavedAt:result.lastSavedAt};
 saveRemoteSession(session); saveFinalReceipt(null); return session;
}
export async function resumeRemoteDraft(session:RemoteSession):Promise<{participant:Participant;responses:Responses;lastSavedAt?:string}>{
 const result=await api<any>({action:'resume',participantId:session.participantId,resumeToken:session.resumeToken});
 const responses:Responses=Object.fromEntries((result.responses||[]).map((r:any)=>[r.device_id,{choice:r.choice,comment:r.comment||undefined}]));
 return {participant:{name:result.participant.name,tower:result.participant.tower,apartment:result.participant.apartment,role:result.participant.role},responses,lastSavedAt:result.participant.lastSavedAt};
}
export async function saveRemoteDraft(session:RemoteSession,responses:Responses):Promise<string>{
 const result=await api<any>({action:'save',participantId:session.participantId,resumeToken:session.resumeToken,responses:serializeResponses(responses)});
 return result.lastSavedAt;
}
export async function finalizeRemoteDraft(session:RemoteSession,responses:Responses):Promise<FinalReceipt>{
 const result=await api<any>({action:'finalize',participantId:session.participantId,resumeToken:session.resumeToken,responses:serializeResponses(responses)});
 const receipt:FinalReceipt={protocolId:result.protocolId,status:'finalized',finalizedAt:result.finalizedAt,payloadHash:result.payloadHash};
 saveFinalReceipt(receipt); saveRemoteSession({...session,status:'finalized'}); return receipt;
}
