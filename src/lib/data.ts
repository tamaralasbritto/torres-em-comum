...
async function api<T>(payload:Record<string,unknown>):Promise<T>{const response=await fetch(API_URL,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});let data:any={};try{data=await response.json()}catch{}if(!response.ok){if(data.error==='unit_already_started')window.alert('Esta unidade já iniciou uma participação. Para continuar com as respostas já salvas, use o mesmo navegador e dispositivo em que a consulta foi iniciada. Se você perdeu esse acesso, fale com a organização antes de criar outra manifestação.');if(data.error==='unit_already_finalized')window.alert('Esta unidade já possui uma manifestação finalizada. Não é necessário fazer um novo cadastro. Se precisar recuperar o comprovante, fale com a organização.');throw new Error(data.error||`http_${response.status}`)}return data as T}

export async function createRemoteDraft(participant:Participant):Promise<RemoteSession>{const turnstileToken=await getTurnstileToken();const result=await api<any>({action:'create',turnstileToken,...participant});const session:RemoteSession={participantId:result.participantId,protocolId:result.protocolId,resumeToken:result.resumeToken,status:'draft',lastSavedAt:result.lastSavedAt};saveRemoteSession(session);saveFinalReceipt(null);return session}
