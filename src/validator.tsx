...
function App(){
 ...
 async function requestOtp(e:React.FormEvent){e.preventDefault();setBusy(true);setMessage('');try{await api('/functions/v1/validator-auth',{email:email.trim().toLowerCase()});setPhase('otp');setMessage('Se este e-mail estiver autorizado, um código de acesso será enviado.')}catch(err){setMessage((err as Error).message)}finally{setBusy(false)}}
 async function verifyOtp(e:React.FormEvent){e.preventDefault();setBusy(true);setMessage('');try{const data=await api('/auth/v1/verify',{type:'email',email:email.trim().toLowerCase(),token:otp.trim()});const access=data.access_token;if(!access)throw new Error('login_failed');sessionStorage.setItem(SESSION_KEY,access);setToken(access);setPhase('app')}catch(err){setMessage((err as Error).message)}finally{setBusy(false)}}
