...
type SaveState='local'|'loading'|'ready'|'saving'|'saved'|'error'|'finalizing'|'finalized';
...
function App(){
 ...
 const [saveState,setSaveState]=React.useState<SaveState>(()=>finalReceipt?'finalized':remoteSession?'loading':'local');
 const [lastSavedAt,setLastSavedAt]=React.useState<string|undefined>(()=>remoteSession?.lastSavedAt);
 const [hydrated,setHydrated]=React.useState(()=>!remoteSession||remoteSession.status==='finalized');
 ...
 const startParticipation=async(newVersion=false)=>{
   ...
   }catch(e){setSaveState('error');alert(String((e as Error).message)==='owner_already_finalized'?'Já existe uma manifestação finalizada pelo proprietário desta unidade.':'Não foi possível iniciar o salvamento seguro agora. Seus dados locais continuam neste navegador.')}
 };
 ...
 const finalize=async()=>{
   ...
   if(!window.confirm('Finalizar esta manifestação? O registro final ficará auditável e não será editado. Se você mudar de ideia depois, poderá criar uma nova versão.'))return;
   setSaveState('finalizing');
   try{
     const receipt=await finalizeRemoteDraft(remoteSession,responses);
     setFinalReceipt(receipt);setRemoteSession({...remoteSession,status:'finalized'});setSaveState('finalized');await refreshPanel();
   }catch(e){setSaveState('error');alert(String((e as Error).message)==='owner_already_finalized'?'Uma manifestação do proprietário já foi finalizada para esta unidade e prevalece sobre a do inquilino.':'Não foi possível finalizar. Tente novamente; o rascunho permanece salvo.')}
 };
 const manifestationText=()=>{
   const proof=finalReceipt?`Protocolo: ${finalReceipt.protocolId}\nHash de integridade (SHA-256): ${finalReceipt.payloadHash}\nFinalizado em: ${new Date(finalReceipt.finalizedAt).toLocaleString('pt-BR')}\n\n`:'';
   ...
   return header+(body||'Nenhum pedido de alteração, exclusão ou observação foi registrado.')+'\n\nDocumento gerado pelo Torres em Comum. O hash acima corresponde ao registro final armazenado para auditoria.';
 };
 const openEmail=()=>{
 ...
 const saveLabel=finalReceipt?'Manifestação finalizada':saveState==='saving'?'Salvando rascunho…':saveState==='saved'||saveState==='ready'?'Rascunho salvo com segurança':saveState==='loading'?'Conectando ao salvamento seguro…':saveState==='error'?'Falha no salvamento remoto — cópia local preservada':'Ainda não iniciado no banco central';
